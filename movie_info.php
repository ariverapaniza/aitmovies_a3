...
<head>
    ...
    $isInWatchlist = false;
    if ($username) {
        $watchlistCheckQuery = "SELECT 1 FROM watchlist WHERE wusername = ? AND wmovie = ?";
        $watchlistCheckStmt = $conn->prepare($watchlistCheckQuery);
        $watchlistCheckStmt->bind_param("ss", $username, $movieTitle);
        $watchlistCheckStmt->execute();
        $isInWatchlist = $watchlistCheckStmt->get_result()->num_rows > 0;
        $watchlistCheckStmt->close();
    }
        ...
        <div class="row mb-4">
            <div class="col-md-12 text-center">
                <form method="post" class="add-to-watchlist-form">
                    ...
                    <button type="button" class="btn btn-primary btn-toggle-watchlist"
                        data-in-watchlist="<?php echo $isInWatchlist ? 'true' : 'false'; ?>">
...
$(document).ready(function() {
    $('.btn-toggle-watchlist').click(function() {
        var $button = $(this);
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                'toggle_watchlist': true,
                'movie_title': movieTitle
            },
            success: function(response) {
                alert(response);
                var isInWatchlist = $button.data('in-watchlist') === 'true';

                $button.data('in-watchlist', !isInWatchlist);

                if (!isInWatchlist) {
                    $button.html('<i class="bi bi-dash"></i> Remove from Watchlist');
                } else {
                    $button.html('<i class="bi bi-plus"></i> Add to Watchlist');
                }
            }
        });
    });
